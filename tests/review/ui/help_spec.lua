...
describe("review.ui.help", function()
  ...
  it("renders current commands and read-only review keymaps", function()
    ...
    assert.is_true(joined:find("focus threads", 1, true) ~= nil)
    assert.is_nil(joined:find("tree/flat files", 1, true))
    assert.is_true(joined:find("blame panel", 1, true) ~= nil)
    assert.is_true(joined:find("toggle blame side", 1, true) ~= nil)
